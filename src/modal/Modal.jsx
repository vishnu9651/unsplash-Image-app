...
export default function ModalBox({ data }) {
    ...
    return (
        <>
            <br />
            <Button onClick={onOpen} m={"auto"} bg={"pink.500"} _hover={{ bg: "blue.500" }} color={"white"}>More Details</Button>
                ...
                <ModalContent>
                    ...
                    <ModalBody mt={"30px"} pb={"20px"}>
                        ...
                        <Flex gap={"5px"} alignItems={"center"} margin={"10px"}>
                            <SlSocialInstagram /> :
                            <Text>{data.user.social.instagram_username == null ? "Not Available" : data.user.social.instagram_username}</Text>
                        </Flex>
                        <Flex gap={"5px"} alignItems={"center"} margin={"10px"}>
                            <SlSocialTwitter /> :
                            <Text>{data.user.social.twitter_username == null ? "Not Available" : data.user.social.twitter_username}</Text>
                        </Flex>
                        <Flex gap={"5px"} alignItems={"center"} margin={"10px"}>
                            <AiOutlineExport /> :
                            <Text overflow={"hidden"}>{data.user.social.portfolio_url == null ? "Not Available" : data.user.social.portfolio_url}</Text>
                        </Flex>
